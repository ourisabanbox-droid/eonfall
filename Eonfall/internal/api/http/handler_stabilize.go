...
func (h *Handler) PostStabilizeRegionAction(w http.ResponseWriter, r *http.Request) {
	...
	var req StabilizeRegionActionRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		http.Error(w, "invalid json body", http.StatusBadRequest)
		return
	}

	if err := ValidateStabilizeRegionActionRequest(req); err != nil {
	...
	action := &world.WorldAction{
		...
		UserID:         req.UserID,
		ActionType:     world.ActionStabilizeRegion,
		State:          world.ActionPending,
		TargetTick:     currentTick + 1,
		Payload: map[string]any{
			"region_id": req.RegionID.String(),
		},
